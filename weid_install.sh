...
function installBuildTools() {
    ...
    if [ ! -d "weid-install-tools" ];then
        ...
        if [ ! -f "weid-install-tools.zip" ]; then
            ...
            if [[ ${repo} == "cn" ]];then
                curl -o weid-install-tools.zip https://osp-1257653870.cos.ap-guangzhou.myqcloud.com/WeIdentity/weid-install-tools.zip
            else 
                curl -o weid-install-tools.zip https://github.com/WeBankFinTech/WeIdentity-Build-Tools/releases/download/v1.0.12/weid-install-tools.zip
            fi
        fi
...
if is_mac; then
    echo "the current version is MAC $version"
    sed -i "" "s|repo=cn|repo=$repo|g" gradle.properties
    sed -i "" "s|buildToolVersion=+|buildToolVersion=$version|g" gradle.properties
